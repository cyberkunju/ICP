...
function seedAttendanceData(PDO $db) {
    // Add attendance records
    $db->exec("INSERT INTO attendance (student_id, subject_id, session_id, attendance_date, status, marked_by) VALUES (1, 1, 1, '2024-01-01', 'present', 2)");
    $db->exec("INSERT INTO attendance (student_id, subject_id, session_id, attendance_date, status, marked_by) VALUES (1, 1, 1, '2024-01-02', 'absent', 2)");
    $db->exec("INSERT INTO attendance (student_id, subject_id, session_id, attendance_date, status, marked_by) VALUES (1, 2, 1, '2024-01-01', 'present', 2)");
}

function seedStudyMaterialsData(PDO $db) {
    // Create the study_materials table
    $db->exec("CREATE TABLE IF NOT EXISTS study_materials (id INTEGER PRIMARY KEY AUTOINCREMENT, department TEXT, semester INTEGER, subject TEXT, material_type TEXT, file_name TEXT, file_path TEXT, file_url TEXT, file_size INTEGER, uploaded_by INTEGER)");

    // Add study materials
    $db->exec("INSERT INTO study_materials (department, semester, subject, material_type, file_name, file_path, file_url, file_size, uploaded_by) VALUES ('Computer Science', 1, 'CS101', 'notes', 'cs101_notes.pdf', '/path/to/notes', '/url/to/notes', 1024, 2)");
}

function seedNoticesData(PDO $db) {
    // Add notices
    $db->exec("INSERT INTO notices (title, content, target_audience, created_by) VALUES ('General Announcement', 'This is a notice for everyone.', 'all', 1)");
    $db->exec("INSERT INTO notices (title, content, target_audience, created_by) VALUES ('Student-specific Notice', 'This is for students only.', 'students', 1)");
    $db->exec("INSERT INTO notices (title, content, target_audience, created_by) VALUES ('Teacher-specific Notice', 'This is for teachers only.', 'teachers', 1)");
}
